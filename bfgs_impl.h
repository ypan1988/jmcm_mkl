 ...
 */
template<typename T>
void BFGS<T>::Optimize(T &func, slab::vec &x, const double grad_tol) {
  int debug = 0;
  ...
  if (debug) {
    std::cout << "Initializing the function value" << std::endl;
  }

  double f = func(x);

  if (debug) {
    std::cout << "Initializing the gradient" << std::endl;
  }
  ...
  slab::mat hess_inv(n_pars, n_pars);
  hess_inv.diag() = 1.0;
  ...
  for (int iter = 0; iter != kIterMax; ++iter) {
    ...
    if (trace_) {
      std::cout << std::setw(5) << iter << ": " << std::setw(10) << f << ": "
                << x.t() << std::endl;
    }
    ...
    slab::vec dg = grad - grad2;
    // Compute difference times current matrix
    slab::vec hdg = slab::matmul(hess_inv, dg);

    // Calculate dot products for the denominators
    double fac = slab::dot(dg, p);
    double fad = 0.0;
    double fae = slab::dot(dg, hdg);
    double sumdg = slab::dot(dg, dg);
    double sump = slab::dot(p, p);
    ...
    if (fac > sqrt(kEpsilon * sumdg * sump)) {
      ...
      slab::vec u = fac * p - fad * hdg;

      hess_inv += fac * slab::matmul(p, p.t()) - fad * slab::matmul(hdg, hdg.t()) + fae * slab::matmul(u, u.t());
    }

    // Calculate the next direction to go
    p = -slab::matmul(hess_inv, grad);
  }
  if (this->message_) {
    std::cerr << "too many iterations in bfgs" << std::endl;
  }
}
